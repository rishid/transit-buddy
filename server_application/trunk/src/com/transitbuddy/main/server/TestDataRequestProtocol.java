		...
		if (commandType != CommandCode.INVALID_COMMAND)
		{
			switch(commandType)
			{
				case GET_NEARBY_STOPS:
					...
					// return the same stops no matter what
				...
				case GET_STOPS:
					LOGGER.debug("Got Get Stops command.");
					...
					if (stopsCommand != null)
					{
						...
						if (routeName != null)
						{
							...
							{
								LOGGER.debug("3");
								TransitTrip trip = new TransitTrip("", tripHeadsign);
								...
								for (int i = 1; i < 3; i++)
								{
									mCalendar.set(Calendar.HOUR, count);
								...
								// red line, Alewife
								LOGGER.debug("Route name = " + routeName + ", headsign = " + tripHeadsign);
								if (routeName.equals("Red Line") && tripHeadsign.equals("Alewife"))
								{
								...
								// red line, Braintree
								else if (routeName.equals("Red Line") && tripHeadsign.equals("Braintree"))
								{
								...
								// green line, Lechmere
								else if (routeName.equals("Green Line") && tripHeadsign.equals("Lechmere"))
								{
								...
								else if (routeName.equals("Green") && tripHeadsign.equals("Boston College Station"))
								{
					...
					if (output != null)
					{
						LOGGER.debug("Returning non-null trip object");
					}
					break;
				...
				case GET_TRIPS:
					...
					if (tripsCommand != null)
					{
						...
						if (routeName != null)
						{
							TransitRoute route = new TransitRoute("",routeName,null);
							if (routeName.equals("Red Line")) // red line
							{
								route.addTrip(new TransitTrip("Alewife","1"));
								route.addTrip(new TransitTrip("Braintree","2"));
								output = route;
							}
							else if (routeName.equals("Green Line")) // green line
							{
						...
						{
							LOGGER.error("Cannot get trips. Route name in command was null.");
						}
					}
					else
					{
