...
public class TransitInfoRequestProtocol
{
	private static final Logger LOGGER = Logger
	    .getLogger(TransitInfoRequestProtocol.class);
	 ...
	 * 
	 * @param command
	 *          The command object specifying which data to retrieve
	...
	public Serializable processCommand(ICommand command)
	{
			...
			if (commandType != CommandCode.INVALID_COMMAND)
			{
				switch (commandType)
				{
					case GET_NEARBY_STOPS:
						output = mTransitSystemDbManager.getNearbyStops(command);
						break;
					...
					case GET_STOPS:
						...
						// We need to check if real-time data is available for this route
						// and direction, if it is
						// we should call the get real time query, else we need to call the
						// static info query
						TransitInfoUpdatorRunner updator = TransitInfoServerRunner.getsUpdator();
						if (updator != null && updator.isRealtimeAvailable(
						    stopsCommand.getAgencyID(), stopsCommand.getRouteName(),
						    stopsCommand.getTripHeadsign()))
						{
							LOGGER.info("Real-time data available for route_name/headsign: "
							    + stopsCommand.getRouteName() + "/"
							    + stopsCommand.getTripHeadsign());
							output = mTransitSystemDbManager.getRealTimeStops(command);
						}
						else
						{
		...
		{
			LOGGER.error("Could not process command type " + commandType.getValue()
