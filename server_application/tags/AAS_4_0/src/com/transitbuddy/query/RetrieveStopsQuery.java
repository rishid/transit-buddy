...
public class RetrieveStopsQuery extends CommandableQuery
{
	...
	/**
	 * @param numTimesPerStop
	 *          the mNumTimesPerStop to set
	 */
	...
	public Serializable getAnswer(ResultSet rs) throws SQLException
	{
		...
		LOGGER.info("Creating new Transit Trip: " + tripHeadsign);
		...
		while (rs.next())
		{
			...
			LOGGER.info("Working on stop: " + stopName);
			if (!currStopName.equals(stopName))
			{
				// New stop, add previous stop
				if (currStopName != null && !currStopName.equals(""))
				{
					LOGGER.info("Found new stop, adding previous Stop to Trip: " + currStopName);
					trip.addStop(new TransitStop(currStopName, stopId, new Coordinate(
					    stopLat, stopLon), stopTimes));
				}				
			...
			try
			{
				// departureTime is local time
				Time departureTime = rs.getTime(STOP_DEPARTURE_TIME_COL);

				// Local Timezone
				Calendar local = new GregorianCalendar();
				local.set(Calendar.HOUR_OF_DAY, departureTime.getHours());
				local.set(Calendar.MINUTE, departureTime.getMinutes());
				local.set(Calendar.SECOND, departureTime.getSeconds());

				// Convert from Local to UTC
				Calendar utcCalender = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
				utcCalender.setTimeInMillis(local.getTimeInMillis());
				LOGGER.info("Adding stop time to stop");
				stopTimes.add(utcCalender.getTime());
			}
		...
		if (currStopName != null && !currStopName.equals(""))
		{
			LOGGER.info("Adding final Stop to Trip: " + currStopName);
			trip.addStop(new TransitStop(currStopName, stopId, new Coordinate(
			    stopLat, stopLon), stopTimes));
		}
