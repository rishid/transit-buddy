...
public class ExpandableListAdapter extends BaseExpandableListAdapter 
{
    ...
    /**
     * This is supposed to be an intelligent add, but it we get the wrong 
     * trip IDs when we go to query for the data.
     * 
     * @param group
     * @param children
     */
    public void addGroupSmart(String group, ArrayList<String> children) 
    {
    	if (!mGroups.contains(group))
    	{
    		mGroups.add(group);
    	...
    	{
    		// group already exists, add the children to the group
    		mChildren.get(mGroups.indexOf(group)).addAll(children);
    	}
    	
    	// quick way to remove duplicates from children
    	Set<String> s = new LinkedHashSet<String>(mChildren.get(mGroups.indexOf(group)));
    	
    	mChildren.get(mGroups.indexOf(group)).clear();
    	mChildren.get(mGroups.indexOf(group)).addAll(s);     
    }
